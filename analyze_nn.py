...
NN_CSV = Path("results/neural_network/prediction_results3.csv")
OUTPUT_IMG = "analysis_nn3.png"

def analyze():
    if not BASELINE_CSV.exists() or not PREDICTION_CSV.exists() or not NN_CSV.exists():
    ...
    df_nn   = df_nn.rename(columns={"Status": "Status_NN", "Nodes_Explored": "Nodes_NN"})
    
    merged = pd.merge(df_base, df_pred, on="Problem_ID", how="outer")
    merged = pd.merge(merged, df_nn, on="Problem_ID", how="outer", suffixes=('', '_NN'))
    ...
    cnt_nn   = (merged['Status_NN'] == 'Solved').sum()
    cnt_curr = (merged['Status_Pred'] == 'Solved').sum()
    
    # updated label to Neural Network
    bars1 = ax1.bar(['Baseline', 'Neural Network', 'Current'], [cnt_base, cnt_nn, cnt_curr], 
                    color=[c_base, c_nn, c_curr])
    ...
    m_nn   = merged['Status_NN'] == 'Solved'

    # neural network solved but baseline failed
    rescue_nn   = len(merged[m_nn & (~m_base)])
    rescue_curr = len(merged[m_curr & (~m_base)])
    # baseline solved but current transformer failed
    base_only   = len(merged[m_base & (~m_curr)])

    # updated labels
    bars2 = ax2.bar(['Baseline Only', 'NN Rescue', 'Current Rescue'], 
                    [base_only, rescue_nn, rescue_curr], 
                    color=[c_unq, c_nn, c_curr])
    ...
    def plot_scatter(ax, col_model_nodes, col_model_status, color, label_name):
        # baseline and model both solved
    ...
    plot_scatter(axs[1, 0], 'Nodes_NN', 'Status_NN', c_nn, "Neural Network")

    # current transformer and baseline
