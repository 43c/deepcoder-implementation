...
def safe_parse_line(line_str):
    ...
    if '|' in line_str:
        ...
        for p in parts:
            ...
            if len(nums) == 1:
                parsed_parts.append(nums[0])
            else:
                parsed_parts.append(nums)
        return parsed_parts
...
def flatten_and_encode(data):
    ...
    if isinstance(data, (list, tuple)):
        for item in data:
            tokens.extend(flatten_and_encode(item))
    else:
        tokens.append(encode_integer(data))
    return tokens
...
def process_entry_data(input_str, output_str):
    ...
    if not isinstance(raw_inputs, list): raw_inputs = [raw_inputs]
    if not isinstance(raw_outputs, list): raw_outputs = [raw_outputs]

    input_ids.extend(flatten_and_encode(raw_inputs))
    input_ids.append(SEP_ID)

    input_ids.extend(flatten_and_encode(raw_outputs))
    input_ids.append(SEP_ID)
...
if __name__ == "__main__":
    ...
    MODEL_PATH = current_script_dir / "models" / "checkpoint-129398"
    ...
    with torch.no_grad():
        for problem_idx in range(num_problems):
            ...
            for i in range(EXAMPLES_PER_PROBLEM):
                ...
                inp_ids, mask = process_entry_data(all_inputs[line_idx], all_outputs[line_idx])
            ...
            tokens = {'input_ids': input_tensor, 'attention_mask': mask_tensor}
